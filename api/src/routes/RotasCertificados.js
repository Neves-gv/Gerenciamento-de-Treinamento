...
router.post('/certificados', autenticarToken, async (req, res) => {
    ...
    try {
        ...
        if (verificarUsuario.rows.length === 0) {
            return res.status(400).json({ error: 'Usuário não encontrado ou inativo. Verifique o id_usuario informado' });
        }

        // Validar se treinamento existe
        const verificarTreinamento = await BD.query(
            `SELECT * FROM treinamento WHERE id_treinamento = $1`,
            [id_treinamento]
        );
        
        if (verificarTreinamento.rows.length === 0) {
            return res.status(400).json({ error: 'Treinamento não encontrado. Verifique o id_treinamento informado' });
        }
...
router.put('/certificados/:id_certificados', autenticarToken, async (req, res) => {
    ...
    try {
        ...
        // Validar se treinamento existe
        const verificarTreinamento = await BD.query(`SELECT * FROM treinamento WHERE id_treinamento = $1`, [id_treinamento]);
        if (verificarTreinamento.rows.length === 0) {
            return res.status(400).json({ error: 'Treinamento não encontrado. Verifique o id_treinamento informado' });
        }
...
router.patch('/certificados/:id_certificados', autenticarToken, async (req, res) => {
    ...
    try {
        ...
        // Validar usuário se estiver sendo atualizado
        ...
        // Validar treinamento se estiver sendo atualizado
        if (id_treinamento !== undefined) {
            const verificarTreinamento = await BD.query(`SELECT * FROM treinamento WHERE id_treinamento = $1`, [id_treinamento]);
            if (verificarTreinamento.rows.length === 0) {
                return res.status(400).json({ error: 'Treinamento não encontrado. Verifique o id_treinamento informado' });
            }
